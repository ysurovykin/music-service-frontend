...
import { EditProfileRequestData } from "../../store/listener.model";
...
export const EditProfileModal = () => {
  ...
  const isLoading = useSelector(listenerSelectors.isEditProfileLoading);
  const isOpen = useSelector(listenerSelectors.isEditProfileModalOpen);
  const listenerCountry = useSelector(userSelectors.country);
  const listenerBirthDate = useSelector(userSelectors.birthDate);
  const listenerGender = useSelector(userSelectors.gender);
  const listenerName = useSelector(listenerSelectors.name);
  const dispatch = useDispatch();
  const closeEditProfileModal = () => dispatch(listenerActions.closeEditProfileModal());
  const editProfile = (request: EditProfileRequestData) => dispatch(listenerActions.editProfile(request));
  ...
  const editProfileDisabled = useMemo(() => {
    ...
    const noChanges = (!profileImageSrc && !profileImageUrl || profileImageSrc === profileImageUrl) && name === listenerName &&
      country === listenerCountry && gender === listenerGender && birthDateDay === listenerBirthDate?.day &&
      birthDateMonth === listenerBirthDate?.month && birthDateYear === listenerBirthDate?.year;
  ...
  useEffect(() => {
    if (listenerName && isOpen) {
      setName(listenerName);
    }
  }, [listenerName, isOpen]);

  useEffect(() => {
    if (listenerBirthDate && isOpen) {
      setBirthDateDay(listenerBirthDate.day!);
      setBirthDateMonth(listenerBirthDate.month!);
      setBirthDateYear(listenerBirthDate.year!);
    }
  }, [listenerBirthDate, isOpen]);

  useEffect(() => {
    if (listenerCountry && isOpen) {
      setCountry(listenerCountry);
    }
  }, [listenerCountry, isOpen]);

  useEffect(() => {
    if (listenerGender && isOpen) {
      setGender(listenerGender);
    }
  }, [listenerGender, isOpen]);
      ...
      <div className="edit-profile-modal__wrapper">
        <div
          ...
          <Avatar
            ...
            style={{ backgroundColor: `${profileImageSrc ? '' : 'rgb(82, 196, 72)'}` }}
            size={174}
            src={profileImageSrc}
            shape="circle">
        ...
        <Form
          ...
          initialValues={{
            ...
            day: listenerBirthDate?.day,
            month: months[(listenerBirthDate?.month!).toString()],
            year: listenerBirthDate?.year
          }}>
          <Row gutter={8}>
            <Col span={24}>
              <Form.Item
